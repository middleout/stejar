...
test("should be able to run a request", async () => {
    ...
    adapter.mock("test.com", Method.GET as any, (requestParam: HttpRequest) => {
...
test("should be able to run with a request interceptor", done => {
    ...
    const http = new HttpService(adapter);
    http.addRequestInterceptor((requestParam: HttpRequest) => Promise.resolve(requestParam));
...
test("should be able to run with a request interceptor that modifies the request", async () => {
    ...
    adapter.mock("example.com", Method.GET as any, (requestParam: HttpRequest) => {
        expect(requestParam.getPath()).toEqual("http://example.com");
        return new Promise(resolve => resolve("I WAS MODIFIED"));
    });
    adapter.mock("blabla.com", Method.GET as any, (requestParam: HttpRequest) => {
        expect(requestParam.getPath()).toEqual("http://blabla.com");
        return new Promise(resolve => resolve("I WAS MODIFIED the 2nd time"));
    });

    const http = new HttpService(adapter);
    http.addRequestInterceptor((requestParam: HttpRequest) => Promise.resolve(new HttpRequest("http://example.com")));
    http.addRequestInterceptor((requestParam: HttpRequest) => Promise.resolve(new HttpRequest("http://blabla.com")));
...
test("should be able to run with a request interceptor but removing it", done => {
    ...
    const http = new HttpService(adapter);
    const remover = http.addRequestInterceptor((requestParam: HttpRequest) =>
        Promise.resolve(new HttpRequest("http://example.com"))
    );
    remover();
    http.send(request).then(response => {
        expect(response.data).toEqual("OK");
...
test("should be able to run with a response interceptor", done => {
    ...
    const http = new HttpService(adapter);
    http.addResponseInterceptor((response: HttpResponse<any>) =>
...
test("should be able to run with 2 response interceptor", done => {
    ...
    http.addResponseInterceptor((response: HttpResponse<any>) =>
        Promise.resolve(new HttpResponse(200, "OK", request, "I AM A 2ND RESPONSE", {}))
    );
    http.send(request).then(response => {
...
test("should be able to run with a request interceptor but removing it", done => {
    ...
    const http = new HttpService(adapter);
    const remover = http.addResponseInterceptor((response: HttpResponse<any>) =>
        Promise.resolve(new HttpResponse(200, "OK", request, "I AM A NEW RESPONSE", {}))
    );
    remover();
    http.send(request).then(response => {
